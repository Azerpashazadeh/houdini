...
# ===== YAYA GECİDİ HDA =====
# HDA Object node'unu bul
obj_node = None
for n in hou.node('/obj').children():
    if 'yaya_gecidi' in n.type().name():
        obj_node = n
        break

if obj_node is None:
    print('HATA: yaya_gecidi node bulunamadi')
else:
    ...
    # ===== PARAMETRELERİ OKU =====
    try:
        ADET        = obj_node.parm('adet').eval()
        GENISLIK    = obj_node.parm('genislik').eval()
        KALINLIK    = obj_node.parm('kalinlik').eval()
        UZUNLUK     = obj_node.parm('uzunluk').eval()
        ARA_MESAFE  = obj_node.parm('ara_mesafe').eval()
        BASLANGIC_Z = obj_node.parm('baslangic_z').eval()
        ROTATE      = obj_node.parm('rotate').eval()
        KONUM_X     = obj_node.parm('konum_x').eval()
        KONUM_Y     = obj_node.parm('konum_y').eval()
        KONUM_Z     = obj_node.parm('konum_z').eval()
    except:
        ADET        = 8
        GENISLIK    = 4.5
        KALINLIK    = 0.02
